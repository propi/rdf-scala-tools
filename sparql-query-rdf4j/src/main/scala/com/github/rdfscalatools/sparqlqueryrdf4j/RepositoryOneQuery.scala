...
/**
  ...
  */
class RepositoryOneQuery[O] private[sparqlqueryrdf4j](implicit actorSystem: ActorSystem, ec: ExecutionContext, materializer: Materializer, repository: Repository, unmarshaller: FromResponseWithAcceptUnmarshaller[O], requester: HttpRequest => Future[HttpResponse])
